...
export const ParsedTimeBadge: React.FC<ParsedTimeBadgeProps> = ({ data }) => {
  ...
  // ✅ 2. hour_window 也要判空、判长度
  const hourRange =
    data.hour_window && data.hour_window.length === 2
  ...
  return (
    <div className="bg-polimi-50 border border-polimi-100 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-fade-in">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-polimi-900 uppercase tracking-wider mb-1">
            Time Understanding
          </h3>
      ...
      <div className="flex flex-wrap gap-3">
        {dates.map((date) => (
          <div
            key={date}
            className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-md border border-slate-200 text-sm font-medium text-slate-700 shadow-sm"
          >
